  ...
  return function (dispatch, getState) {
    ...
    db.saveCharacterTemplate(name, selectedCharacters,
      ...
      error => dispatch(showFlash(
        'Erreur de stockage',
        'Erreur lors de la sauvegarde du modèle de personnage : ' + error.message + '. Veuillez réessayer.'
      ))
    );
  };
}
...
export function saveTemplates(templates) {
  ...
  return function (dispatch) {
    db.saveCharacterTemplates(
      ...
      error => dispatch(showFlash(
        'Erreur de stockage',
        'Erreur lors de la sauvegarde des modèles de personnage : ' + error.message + '.'
      ))
    );
  };
}
      ...
      (dispatch, getState, newProfile) => {
        ...
        if (missingCharacters.length) {
          dispatch(showFlash(
            ...
            'Les personnages suivants sont absents du modèle sélectionné : ' + missingCharacters.join(', ')
          ));
        }
      }
    )
  ...
  return function (dispatch, getState) {
    if (Object.keys(defaultTemplates).includes(name)) {
      ...
      db.getCharacterTemplate(
        ...
        error => dispatch(showFlash(
          'Erreur de stockage',
          `Erreur lors de la récupération de votre modèle depuis la base de données : ${error.message}.`
        ))
      );
    }
  }
...
export function deleteTemplate(name) {
  ...
  return function (dispatch) {
    db.deleteCharacterTemplate(
      ...
      error => dispatch(showFlash(
        'Erreur de stockage',
        `Erreur lors de la suppression du modèle de personnage '${name}'. Message d'erreur : ${error.message}`
      ))
    );
  }
}
