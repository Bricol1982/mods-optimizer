  ...
  return function (dispatch) {
    ...
    db.delete(
      ...
      error => dispatch(showError(
        'Erreur lors de la suppression de la base de données : ' + error.message + '. Essayez de la vider manuellement et d’actualiser la page.'
      ))
    );
  };
}
...
export function restoreProgress(progressData) {
  return function (dispatch) {
    try {
      ...
      throw new Error(
        'Impossible de traiter le fichier de progression. Est-ce un fichier modèle ? Dans ce cas, utilisez le bouton "charger" ci-dessous. Message d’erreur : ' +
        e.message
      );
    }
  }
  ...
  return function (dispatch) {
    ...
    db.deleteProfile(
      ...
      error => dispatch(showFlash(
        'Erreur de stockage',
        'Erreur lors de la suppression de votre profil : ' + error.message
      ))
    );
  };
}
...
export function updateProfile(updateFunc, auxiliaryChanges = nothing) {
  return function (dispatch, getState) {
    ...
    db.saveProfile(
      ...
      error => dispatch(showFlash(
        'Erreur de stockage',
        'Erreur lors de la sauvegarde de votre progression : ' + error.message + ' Votre progression pourrait être perdue après l\'actualisation de la page.'
      ))
    );
